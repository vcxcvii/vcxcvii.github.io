...
```bash
python3 _scripts/generate-og.py
ruby _scripts/validate-posts.rb
ruby -c _scripts/qa.rb
ruby -c _scripts/validate-posts.rb
node --check assets/js/gh-graph.js assets/js/changelog.js assets/js/days.js assets/js/theme.js
...
Rendered pages include unique titles and descriptions, canonical URLs, breadcrumbs with `BreadcrumbList` schema, crawl directives, RSS discovery, and JSON-LD for `Person`, `WebSite`, page type, and `BlogPosting`. Legacy archive aliases are `noindex` and canonicalize to `/blog/`.

Every post gets a 1200x630 Open Graph banner at `assets/images/og/<slug>.png`, generated by `_scripts/generate-og.py` and resolved automatically from the post slug. Run the generator after adding a post, or the share card falls back to the site default.
