...
- **The ramp is three stops per ground,** `--bar-a`, `--bar-b`, `--bar-c`. The midpoint sits at `30%`, not halfway: white text measures `4.19:1` against the top stop and `7.85:1` against the middle one, so the light band has to finish above the text rather than behind it.
- **Chroma sits at the edge of the sRGB gamut** for each stop's lightness, `0.220`, `0.218`, `0.216` at hue `262.5°`. The first ramp inherited its chroma from the purple it was rotated out of, around `0.10`, and a blue at half the available saturation reads as grey-blue chrome rather than as the site's blue. Lightness is what carries the contrast, so saturating the ramp cost nothing.
- **Beige gets its own ramp but not its own hue.** Pulling it to `236°` softened it against warm paper and also capped the chroma, since the gamut is narrow there: `0.115` against `0.220`. The bar came out teal. Beige now sits at `262.5°` like the others, at 88% of gamut width rather than the full amount, which is the whole of the concession warm paper gets.
