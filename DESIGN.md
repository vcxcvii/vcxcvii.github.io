...
- No cards, pills, tab bars, shadows, decorative animation, fake browser chrome, CRT effects, or nostalgia cosplay. One glass effect is permitted and it is the masthead: `backdrop-filter: saturate(200%) blur(30px)` behind a `.55` scrim, pushed toward Apple's Liquid Glass treatment on top of the `#globalnav` starting point; see **Masthead**. It left `FORBIDDEN_PATTERNS` in `_scripts/qa.rb` for that and nothing else. Gradients are now permitted nowhere: the masthead ramp was the last one and the scrim replaced it. `radial-gradient` and `box-shadow` stay forbidden.
...
- **The Ask row lives under the MCP callout, on every page that has one.** Six marks, ChatGPT, Claude, Perplexity, Gemini, Grok and Copilot, listed in `_data/ask_ai.yml`. It moved up from the footer, where nobody scrolled to it, and was moved rather than copied: the six marks are about 11KB inline, and Gemini and Copilot carry fixed gradient ids that would collide with a second copy on the same page.
- Marks are `1.25rem` inside `2.75rem` (44px) targets with no gap; the targets themselves are the spacing. The row is pulled `-.75rem` left, half the difference between mark and target, so the first mark sits on the text's left edge, and all six hold one row at `320px`. Links take `color: var(--color-text)` so OpenAI and Grok, the two `currentColor` marks, invert with the ground.
- **The callout line ends with a quip that points at the row, and it is per page.** `ask_quip` in front matter wins as a forced static line (used on pages with page-specific flavour: `404`, `changelog`, `privacy`, `contact`, `disclaimer`, `about`, `blog/`, `side-quests/`, `days/`, `work/`, `speaking/`, `uses-this`). Every other page rotates through `_data/ask_quips.yml`: the build picks one by day of year, so the page still changes without JavaScript, and `assets/js/quips.js` (loaded on every page, budgeted at `1,000` bytes) picks a fresh one per visit and never repeats the last one in the tab. A new line has to read correctly after `Or tap one below.` on any page — nothing can name a specific page type (essay, project, day), and nothing in the pool jokes about an employer.
- **Prompts are page-aware and carry the page URL.** Essays ask for the argument, the key points and where to push back; every other page asks who VC is, what he has worked on and what he writes about, and to cite the pages it used. The URL is what makes an assistant read the site instead of answering from its training data about a name.
- **Each assistant gets the surface that reliably takes `q=`.** ChatGPT carries `hints=search`, which forces a web search, so it reads the page instead of answering from memory. Gemini goes to Google AI Mode (`udm=50`) and Copilot to Bing Copilot Search, because neither `gemini.google.com` nor `copilot.microsoft.com` documents a prefill parameter.
- **Where the three newer marks come from.** Gemini is Google's 2025 multi-colour star from Wikimedia Commons, with its no-op `feFlood`/`feBlend` pairs dropped. Grok is the mark cut from grok.com's own logo SVG and takes `currentColor` as a near-black. Copilot is Microsoft's 2026 two-tile gradient mark. Simple Icons has none of the three in colour: Gemini is there only flat, and Grok and Microsoft Copilot are not there at all.
...
- **The bar is a translucent scrim, read off apple.com and then pushed toward Liquid Glass.** `background: rgba(250, 250, 252, .55)` with `-webkit-backdrop-filter` and `backdrop-filter: saturate(200%) blur(30px)`, ink at `rgba(0, 0, 0, .8)` going solid on hover, and `44px` tall. Dark mirrors it at `rgba(22, 22, 23, .55)` over `rgba(255, 255, 255, .8)`. The starting values were `#globalnav`'s own from `globalheader.css` (alpha `.8`, `saturate(180%) blur(20px)`); those held two revisions and then moved to match the Liquid Glass tuning Apple shipped after: alpha lower, blur wider, saturation higher, so what shows through reads as content refracted through a pane rather than diffused behind frost. A `@supports not (backdrop-filter: blur(1px))` block falls the bar back to `--color-bg` on browsers without support, because a `.55` scrim without a blur is not a scrim.
...
- **Adding a mark is a fetch, in this order.** Read this section first. Then `https://cdn.jsdelivr.net/npm/simple-icons/icons/<slug>.svg` for the path, and the brand `hex` from the Simple Icons data for the fill (older releases such as `simple-icons@14` carry the data for a brand the latest release has dropped from it). Normalise as above. If Simple Icons has nothing, the product falls through to the letter fallback. **Never hand-draw a glyph for a named product.** `_scripts/qa.rb` fails on any stroke glyph in `_includes/logos/` that is not on its short list of category glyphs, so a new invented icon cannot ship quietly.
- **Google's marks are multi-colour and do not come from Simple Icons.** Simple Icons carries Chrome, Analytics and the Google G as one flat colour, which is a different logo. Chrome and the Google G come from their Wikimedia Commons SVGs, Analytics and Tag Manager from Google's own `fonts.gstatic.com/s/i/productlogos/<name>/v<n>/192px.svg`, and Search Console is the icon cropped out of Google's published logo by `viewBox`, dropping the wordmark paths. Google Workspace publishes a wordmark only, so its row shows the Google G. Every multi-colour mark keeps its native `viewBox`, has `width`, `height` and `enable-background` stripped, and gets its gradient and clip ids prefixed with the mark's name, because inline SVGs share one id namespace and a bare `id="a"` in two marks paints the wrong gradient.
- **Paper is the glyph from its own published wordmark.** context.dev returns Paper as a 512px PNG and a `110x30` wordmark SVG that carries the glyph as its second path. That path is the mark: taken as is, scaled `.923` into the `24` box, `#81acec`.
- **A product with no published mark falls through to the letter fallback,** and that is the designed answer rather than a gap to fill. cmux, CodexBar, Quartr, Screen Studio, Mole, SwanAI, Clay, Apollo, KitAI, Podpitch and Logitech have nothing to fetch. `_includes/logo.html` is regenerated from the directory listing whenever a mark is added or removed, so the case statement and the files cannot drift apart.
...
- context.dev's `/brand/retrieve` is not a source for these. It answers with PNG or JPG for most brands, its Letterboxd "SVG" is an HTML `<foreignObject>` wrapper, and its Cursor icon is a 4.4KB gradient build of a mark that inlines in 454 bytes. It is a brand-intelligence API, not a logo CDN. Retested on 2026-09-21 with the site owner's key: it still returns PNG for most products (Chrome, Analytics, Tag Manager and Search Console all come back as a 128px "Google" icon), and the one SVG it returns is a wordmark. It stays a source of last resort, useful for a brand's colours and for finding a glyph inside a wordmark, as with Paper.
...
- Labels are Title Case, matching the reference nav: `About`, `Work`, `Blog`, `Side Quests`, `Speaking`, `Days`, `Uses This`, `Contact`. They were lowercase for the terminal type system and changed with it.
...
- Mobile: `VC` and a plain hamburger occupy the first line. Opening it reveals an opaque panel over the page content, never changing document flow or pushing content down. The panel uses one hairline, no shadow or animation.
- **Panel rows are full width and left-aligned, at `1rem`.** They were right-aligned at the bar's `.75rem`, which left the whole left half of the sheet empty and made the tap target only as wide as the word: `Blog` gave four characters to hit. The row is the target now, which is what every mobile menu does, and twelve pixels is a size for a dense desktop bar rather than for the only list on the screen.
- **`.site-links` has to reset `margin-left: 0` in the panel.** The desktop rule uses `margin-left: auto` to push the group to the right pole, and inside a column flex container that shrink-wraps the list to the right edge and silently defeats `align-items: stretch`. This is the failure above, and the fix is one declaration.
- `contact` and the page light share the last row at `space-between`. The light sat at the far edge with nothing near it and read as a stray dot.
...
- **One positioning line sits under the name,** `.lede`, at `1.125rem` weight `700` with `text-wrap: balance`. The `h1` carries the initials, `Hi, I'm Varun Choraria (VC).`, which is what retired the `VC for short.` that used to open the first paragraph: the aside was doing work the heading could do for free. It holds one row at desktop width and wraps on mobile, which is why it keeps `text-wrap: balance`: headings get `balance` sitewide and this is a heading in everything but tag.
- **The opening `Hi` rotates through greetings in other languages,** with the English translation in parentheses, e.g. `Namaste (Hi), I'm Varun Choraria (VC).` The pool lives in `_data/hi.yml`: the build picks one by day of year so the page still changes without JavaScript, and `assets/js/hi.js` (loaded on `/` only, budgeted at `1,000` bytes) picks a fresh one per visit and never repeats the last one in the tab. The greeting sits inside `<bdi>` so RTL scripts like `مرحبا` and `שלום` isolate from the fixed LTR remainder. A new line has to read correctly with the comma directly after it; English is one of the lines and stays plain, without the parenthetical.
...
- Empty days are a 10% tint of `var(--color-text)`. Busier days use `var(--color-accent)` at 35%, 65% and 100%, mixed into transparent with `color-mix`, never `opacity` (it would dim the hover tooltip with the cell) and never the GitHub green ramp, which carries contribution counts on the home page and must not mean two things. Cells are anchors, so they must not read `currentColor`, which resolves to the link colour.
- The grid is `aria-hidden`. Every cell carries its date and count as a `title` for pointer users, and the summary and week list carry the same information in text, so strength is never the only carrier. The caption says stronger rather than darker, because opacity over ink inverts with the ground.
- Month ticks sit above the grid, each label spanning as many grid columns as its month has weeks. Both grids share one column template (a `1.7rem` weekday-label column, then `minmax(0, .9rem)` per week), so they align without a second measurement or any JavaScript. A month with one week gets no label. A four-step legend runs quiet to busy, then a rule closes the block before the weeks begin.
...
- Density bands are 1, 2, and 3 or more items on a day. A normal day here lands one thing; wider bands would render every busy day the same shade.
...
- Six plain-language groups: `Work`, `Read`, `Free tools`, `AI`, `About`, and `Site`.
...
- Desktop uses a four-column grid inside the `980px` container, with the fifth and sixth groups continuing onto the second row. Mobile uses two columns and stays at two below `30rem`: six groups of five links in one column is a screen and a half of footer on a phone, and these labels hold half of a `320px` screen. Links stack vertically without dot separators.
- `site design` links to the readable canonical file in the public GitHub repository.
- `Ask` is no longer in the footer. It moved to the MCP callout; see **MCP callout** under Typography.
- Final row sits below its own hairline and splits: copyright and `Built with AI, supervised by a human.` at one end, region at the other. `flex` with `space-between`, not the grid above it, because two items going to opposite ends is what `space-between` is for. Below `44rem` it stacks, because `space-between` needs the desktop width and without it both halves squeeze into thin columns and wrap.
...
- British spelling throughout page copy, matching `inLanguage: en-IN`: `-ise`, `colour`, `centre`, `catalogue`, `judgement`, `behavioural`. Code, CSS properties, file names and command flags keep their real spelling, so `color-systems` stays as it is.
...
- Every page carries an `intro` (About is the one exception: `no_intro: true`, because its opening paragraph is the standfirst, and QA still requires its `description`), the visible standfirst the page layout renders under the H1, and `_scripts/qa.rb` fails the build without one. It is the site's summary mechanism: the reader sees it, and it is what a machine reads when no separate `description` exists. Essays use `description` instead, which feeds both the meta tag and `BlogPosting.description`.
