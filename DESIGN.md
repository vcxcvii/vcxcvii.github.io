...
- **The ramp is GitHub's blue,** steps 5, 6 and 7 of the Primer scale, read from `primer/primitives` at `src/tokens/base/color/{light,dark}`. Light is `#0969da`, `#0550ae`, `#033d8b`; dark is `#1f6feb`, `#1158c7`, `#0d419d`. `#0969da` is Primer's accent foreground, the blue on every GitHub link, which is the one that reads as GitHub blue rather than as some blue.
- **Steps 4, 5, 6 would be the obvious ramp and they fail.** White nav links at `opacity: .8` measure `3.89:1` over `#0969da` as a midpoint. One step down puts the midpoint on `#0550ae`: links at `5.45:1`, white on the top stop at `5.19:1`, white on the middle at `7.59:1`.
