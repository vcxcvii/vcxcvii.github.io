...
- The masthead is the one deliberate exception, and it is blue, not green. It uses Primer's blue scale directly; see **Masthead**. The greens stay reserved, so the graph is still the only thing on the page speaking in GitHub green.
...
- **The ramp is GitHub's blue,** steps 4, 5 and 6 of the Primer scale, read from `primer/primitives` at `src/tokens/base/color/{light,dark}`. Light is `#218bff`, `#0969da`, `#0550ae`; dark is `#388bfd`, `#1f6feb`, `#1158c7`. `#0969da` is Primer's accent foreground, the blue on every GitHub link, which is the one that reads as GitHub blue rather than as some blue.
- **This is the lightest the bar goes while its text is white,** and the number that decides it is not the contrast against any one stop. On a `45px` bar with `.8125rem` text the cap band runs from `33%` to `67%` of the height, entirely past the midpoint, so only the middle and bottom stops are ever behind a glyph. Measured across that band the worst point is `5.27:1` light and `4.71:1` dark. Steps 3, 4, 5 put it at `3.47:1`.
...
- **A genuinely pale bar means dark text.** Steps 0, 1, 2 with `#0a3069` on them measures `7.32:1` and is a real option, but it is a different object: a soft banner rather than a masthead, and at `1.14:1` against white paper the top of it disappears into the page.
- **Three stops, midpoint at `30%`.** The bar reads flat without a highlight at the top, and the links have to sit over the middle stop rather than the lightest one. That is the whole reason the midpoint is not at halfway.
