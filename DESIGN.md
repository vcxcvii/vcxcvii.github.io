...
- **Letterboxd is hand-built,** three circles at `#ff8000`, `#00e054` and `#40bcf4`. Simple Icons carries it as one monochrome path, and a one-colour Letterboxd is a different logo: the three lenses are the mark.
...
- Marks use the real brand logo wherever one stays legible at `20px`. Apple devices use device glyphs, because one repeated Apple mark tells the reader less than a laptop, watch, and phone do. See **Brand marks** for where the paths and the colours come from.
