...
- `Ask` uses brand-colour SVG logos for ChatGPT, Claude, Perplexity, Gemini, Grok and Copilot, each inside an accessible 44px link target, `.25rem` apart so all six hold one row down to a `320px` screen (`flex-wrap` is the fallback, not the layout). No visible text links. Gemini is Google's 2025 multi-colour star from Wikimedia Commons, with its no-op `feFlood`/`feBlend` pairs dropped; Grok is the mark cut from grok.com's own logo SVG and takes `currentColor` as a near-black; Copilot is Microsoft's 2026 two-tile gradient mark. None of the three is in Simple Icons in colour: Gemini is there only flat, Grok and Microsoft Copilot not at all.
