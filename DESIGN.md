...
- **The callout line ends with a quip that points at the row, and it is per page.** `ask_quip` in front matter wins; otherwise essays, side-quest project pages and everything else each get their own default. The homepage rotates through the twenty lines in `_data/ask_quips.yml`: the build picks one by day of year, so the page still changes without JavaScript, and `assets/js/quips.js` (loaded on `/` only, budgeted at `1,000` bytes) picks a fresh one per visit and never repeats the last one in the tab. A new line has to read correctly after `Or tap one below.`, and nothing in the pool jokes about an employer.
